...
//    		remoteViews.setTextViewText(R.id.WordB, list.get(index[1]));
    		...
    		edit.putString("wordA", putWordA);
    		edit.putString("wordB", putWordB);
    		edit.commit();


            // もう一回クリックイベントを登録(毎回登録しないと上手く動かず)
//            remoteViews.setOnClickPendingIntent(R.id.NextButton, Widget.clickNextButton(context));
//            remoteViews.setOnClickPendingIntent(R.id.SaveButton, Widget.clickSaveButton(context));

            Widget.pushWidgetUpdate(context.getApplicationContext(), remoteViews);
        }else if(intent.getAction().equals("SAVE_WIDGET")){
            RemoteViews remoteViews = new RemoteViews(context.getPackageName(), R.layout.widget);
            
            SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences(context);
    		
    		String wordA = pref.getString("wordA", "");
    		String wordB = pref.getString("wordB", "");
    		...
    		try {
    			...
    			Toast.makeText(context, "すでに登録された単語です", Toast.LENGTH_SHORT).show();
